...
layui.define([ 'element', 'jquery' ], function(exports) {
  ...
  Tab.prototype.tabAdd = function(_this) {
    ...
    const that = this;
    const closed = that.tabConfig.closed,
    ...
    if (_this.find('i.iconfont,i.layui-icon').attr('data-icon') != undefined) {
      ...
      if (that.hasTab(_this.find('cite').text()) == -1 && _this.siblings('dl.layui-nav-child').length == 0) {
        if ($('.layui-tab-title.top_tab li').length == openTabNum) {
          layer.msg('You can only open ' + openTabNum + 'tab at the same time, otherwise the system will get stuck！');
          return;
        }
        tabIdIndex++;
  ...
  $('body').on('click', '.top_tab li i.layui-tab-close', function() {
    ...
    const curmenu = window.sessionStorage.getItem('curmenu') == 'undefined' ? 'undefined' : window.sessionStorage.getItem('curmenu') == '' ? '' : JSON.parse(window.sessionStorage.getItem('curmenu'));
